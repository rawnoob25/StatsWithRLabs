...
samp1%>%summarise(xbar=mean(area),sd=sd(area),med=median(area),iqr=IQR(area),sampleQ1=quantile(area,probs=0.25),sampleQ3=quantile(area,probs=0.75),sampleMin=min(area),sampleMax=max(area))
```
...
sample_means50 <- ames %>%
                    ...
                    summarise(x_bar = mean(area)) #The result of this computation is that sample_means50 holds a vector of 15000 sample means- each mean is the mean area of a distinct sample of size 50 drawn from the ames population
...
numElements<-sample_means50%>%nrow
numElements
distrOf_sample_means50<-sample_means50%>%summarise(center=mean(x_bar),stdDevOfDistr=sd(x_bar))
distrOf_sample_means50
ggplot(sample_means50,aes(x=x_bar))+geom_histogram(binwidth=35)
...
sample_means_small<-ames%>%rep_sample_n(size=10,reps=25,replace=T)%>%summarise(x_bar=mean(area))

#sample_means_small is a data frame that holds a vector of 25 sample means. Each sample means is the mean area for a distinct sample of size 10 drawn from the ames population. There should be 25 observations.

sample_means_small%>%nrow
...
sample_50_price<-ames%>%rep_sample_n(size=50,reps=10000,replace=T)%>%summarise(x_bar=mean(price))
...
```{r price-sampling}
sample_means50<-ames%>%rep_sample_n(reps=5000,size=50,replace=T)%>%summarise(xbar_price=mean(price))

centAndSpreadOfSample_Means50<-sample_means50%>%summarise(center=mean(xbar_price),stdev=sd(xbar_price))

centAndSpreadOfSample_Means50
#ggplot(sample_means50,aes(x=xbar_price))+geom_histogram(binwidth=3000)
```
Answer to above: I'd guess the mean home price to be about $180,800
...
sample_means150<-sample_means50<-ames%>%rep_sample_n(reps=5000,size=150,replace=T)%>%summarise(xbar_price=mean(price))
centAndSpreadOfSample_Means150<-sample_means150%>%summarize(center=mean(xbar_price),sstdev=sd(xbar_price))
centAndSpreadOfSample_Means150
```
Answer to above: this distribution of sample mean prices using samples of size 150 should have roughly the same center as the distribution of sample mean prices using samples of size 50.

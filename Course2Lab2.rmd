...
```{r}
nrow(ci%>%filter(capture_mu=="yes"))/nrow(ci)
```
...
```{r plot-99-perc-cis}
##BELOW CODE CHUNK GENERATES 95% CONFIDENCE INTERVALS FROM BOUNDS OF 95% CONFIDENCE INTERVALS ALREADY PRESENT IN VARIABLE ci, STORES IN DATA FRAME ciIncl99 AND SPITS OUT A TABLE for ciIncl99

ciIncl99<-ci%>%mutate(estimate=(lower+upper)/2,stdev=(upper-lower)/z_star_95)
conf<-.99
bdProb<-conf+(1-conf)/2
z_star_99<-qnorm(bdProb)
ciIncl99<-ciIncl99%>%mutate(lower99=estimate - z_star_99*stdev,upper99=estimate + z_star_99*stdev)
ciIncl99<-ciIncl99%>%rename(lower95=lower,upper95=upper,capture_mu_95=capture_mu)%>%mutate(capture_mu_99=ifelse(params$mu>lower99 & params$mu<upper99,"yes","no"))


print(ciIncl99%>%nrow) # total number of samples taken (and thus 99% confidence intervals created)

print(ciIncl99%>%filter(capture_mu_99=="yes")%>%nrow) # number of 99% confidence intervals generated by each of the 50 sample statistics that capture the population parameter

kable(ciIncl99)

##BELOW CODE CHUNK RESHAPES THE DATA FRAME ciIncl99 SO THAT LOWER AND UPPER BOUNDS OF EACH CONFIDENCE INTERVAL OCCUPY SEPARATE OBSERVATIONS, BUT SHARE A COMMON KEY (id) AND STORES IN DATA FRAME ciIncl99ForPlot

ciIncl99ForPlot<-data.frame(id=c(1:nrow(ciIncl99),1:nrow(ciIncl99)),bound=c(ciIncl99$lower99,ciIncl99$upper99),capture_mu_99=c(ciIncl99$capture_mu_99,ciIncl99$capture_mu_99))

kable(ciIncl99ForPlot)


##BELOW CODE CHUNK MAKES PLOT OF CONFIDENCE INTERVALS REPRESENTED BY DATA IN ciIncl99ForPlot

ggplot(data=ciIncl99ForPlot,aes(x=bound,y=id,group=id,color=capture_mu_99))+geom_point(size=2)+geom_line()+ #connect pair of points in each id group with lines 
geom_vline(xintercept= params$mu, color="darkgray") #draw a vertica line
